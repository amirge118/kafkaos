...
**What was built**: Added Jaeger (`jaegertracing/all-in-one`) to `docker-compose.yml`, chosen deliberately for its small footprint given this project's recurring memory ceiling. `src/shared/tracing.ts` provides `injectTraceHeaders()` (turns the current span into a W3C `traceparent` Kafka message header) and `extractTraceContext()` (reconstructs the parent context from an incoming message so a new span attaches as a child). Built a parallel traced version of Stage 6's four services in `src/stage18-tracing/`, each extracting the incoming context, starting a child span, and injecting a new context into whatever it produces downstream. See [src/stage18-tracing/](../../src/stage18-tracing/).
