...
**What was built**: four production-shaped services (`order-service`, `payment-service`, `inventory-service`, `shipping-service`) combining all four patterns, a Grafana dashboard and Prometheus alert, a real `kill -9` incident demo, a batched-commit load-test variant proving 2,000,000 messages end-to-end, and a ClickHouse analytics sink. This is the project's main deliverable — see [`../../ARCHITECTURE.md`](../../ARCHITECTURE.md) for the full, self-contained write-up, and [src/stage25-capstone/](../../src/stage25-capstone/) for the code.
