...
**What was built**: Added a `schema-registry` service (`confluentinc/cp-schema-registry:7.6.1`, `localhost:8081`) wired into Kafka UI, plus `src/stage07-schema-registry/` containing three Avro schemas for `Order` (v1, a compatible v2 adding `discountCode`, and a deliberately breaking v3 adding `shippingAddress` with no default), a shared registry client, and Avro producer/consumer scripts. Also hardened `producer-avro.ts` with a `try/catch` around `registry.encode()` after discovering an unhandled schema-mismatch error would kill the whole batch mid-run. See [src/stage07-schema-registry/](../../src/stage07-schema-registry/).
